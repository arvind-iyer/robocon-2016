                        ...
                        switch(globalState){
                                    ...
                                    case FINISHEDSLOPE:
                                        switch(fullWhite){
                                            ...
                                            case 1:
                                                if((river || gameZone == LIGHTGREENZONE) && !passedRiver)
                                                    {
                                                        ...
                                                        switch(side){
                                                            case REDSIDE:                                                               
                                                                servo_control(BAJAJ_SERVO, SERVO_MICROS_MID - 150 - (determine_velocity(ENCODER1) * 10));
                                                            break;
                                                            case BLUESIDE:
                                                                servo_control(BAJAJ_SERVO, SERVO_MICROS_MID + 150 + (determine_velocity(ENCODER1) * 10));
                                                            break;                                                            
                                                ...
                                                else if(passedDownSlope && (get_minimize_count(ENCODER1)> 15) && determine_velocity(ENCODER1)){
                                                    START_UP_play;
                            ...
                            case ENTER_RIVER: //Right before locking the angle with IMU
                                ...
                                if(!read_infrared_sensor(infrared1)){
                                    switch(side){
                                        case REDSIDE:
                                            ardu_cal_ypr[0] = (float)(IMU_ANGLE1 - (determine_velocity(ENCODER1) * (float)5));
                                        break;
                                        case BLUESIDE:
                                            ardu_cal_ypr[0] = (float)(IMU_ANGLE1 + (determine_velocity(ENCODER1) * (float)5));
                                        break;
                            ...
                            case DOWN_SLOPE: //End game, make it turn extreme right / left for the hybrid to grip propeller
                                goNormal();
                                if((get_minimize_count(ENCODER1) > 10) && (determine_velocity(ENCODER1) < (float)0.1)){
