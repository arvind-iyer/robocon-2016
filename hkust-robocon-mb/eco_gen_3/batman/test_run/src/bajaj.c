...
void goScanRiver(void){
    const int RIVER_TURNING_LEFT = 1250;
    const int RIVER_TURNING_RIGHT = 1650;
    float factor = ((begin + end)/ 2) / (float) 16;
    lastMovement = (SLOPE_TURNING_LEFT) - (factor * (RIVER_TURNING_LEFT - RIVER_TURNING_RIGHT));
    servo_control(BAJAJ_SERVO, lastMovement);
}

void goScanRiver2(void){
	const int RIVER_TURNING_LEFT = 950;
    const int RIVER_TURNING_RIGHT = 1950;
    if ((((begin + end)/ 2) + 6) > 15) {
...
void goRiver(void){
    //Stopping condition
    if(get_minimize_count(ENCODER1) > 20 && !read_infrared_sensor(infrared2)){
        reset_encoder_1();
        strcpy(globalStateString,"EXIT RIVER");
        START_UP_play;
        globalState = EXIT_RIVER;
