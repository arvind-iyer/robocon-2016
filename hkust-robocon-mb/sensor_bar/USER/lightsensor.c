...
void DataTransmit(void){
	...
	//if(last_white_pos != white_pos){
        ...
        TxMsg2 = Light_Sensor_Bar_Encoding2();
		Tx_MailBox = CAN_Transmit(CAN1, &TxMsg);    //transmit the message
        Tx_MailBox2 = CAN_Transmit(CAN1, &TxMsg2);
    //
		while(CAN_TransmitStatus(CAN2,Tx_MailBox2)!=CANTXOK && CAN_TransmitStatus(CAN1,Tx_MailBox) != CANTXOK && retry--){
		}
		//last_white_pos = white_pos;
        for(int i = 0; i < 16 ; i++){
            last_white_position[i] = whiteline_position[i];
        }
	//}
}
...
void CaptureCalibrationData(Colour colour, Colour light_colour){
	...
	GPIO_SetBits(GPIOB, GPIO_Pin_5 | GPIO_Pin_6 | GPIO_Pin_7 | GPIO_Pin_8 | GPIO_Pin_9 | GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15);
