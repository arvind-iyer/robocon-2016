...
u32 get_adc(u8 i){
	ADC_SoftwareStartConvCmd(ADC1, ENABLE);	//start the ADC conversion each time this function is called
	while(DMA_GetFlagStatus(DMA1_FLAG_TC1) != SET); //wait for DMA is finished
	return adc_value[i];
}
#endif