...
int main(void) {
	...
	while (1) {
		...
		tft_prints(0, 9, "Time: %u",
				(unsigned int) (get_full_ticks() - TARGET_TICKS)
						/ (uint16_t) 1000);
		tft_update();
				
		uart_tx(COM1, "DISTANCE|%d|%d\n", 2, 4);
	}
}
...
void handleCommand(char * command) {
	...
	case 1: // Testing
		if (ROBOT_MOVING == 0) {
			initTargetQueue(32);
			queueTarget(
					(Target) {.x = 0, .y = 1000, .angle = 0, .curveFactor = 0});
			queueTarget(
					(Target) {.x = 1000, .y = 1000, .angle = 0, .curveFactor = NO_CURVE});
			queueTarget(
					(Target) {.x = 0, .y = 0, .angle = 0, .curveFactor = NO_CURVE});
