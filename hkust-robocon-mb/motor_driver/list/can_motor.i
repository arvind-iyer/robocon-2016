#line 1 "user\\can_motor.c"
#line 1 "user\\can_motor.h"
...
#line 2 "user\\can_motor.c"


static s32 can_motor_encoder_value[16] = {0};

void can_motor_init(void)
{
	can_rx_add_filter((0x0B0 + (u8)MOTOR2), 0x7FF, 	motor_cmd_decoding);
}
	...
	switch (msg.Data[0]) {
		case 0xAA:
			if (msg.DLC == 6) {
				...
				CLOSE_LOOP_FLAG loop_flag = (CLOSE_LOOP_FLAG) msg.Data[5];
	...
	switch (msg.Data[0]) {
		case 0x22:
			if (msg.DLC == 5) {
				...
				if (msg.StdId >= 0x0B0 && msg.StdId < 0x0B0 + 16) {
					s32 feedback = n_bytes_to_one(&msg.Data[1], 4);
					can_motor_encoder_value[msg.StdId - 0x0B0] = feedback;
				}
			}
		break;
	}
