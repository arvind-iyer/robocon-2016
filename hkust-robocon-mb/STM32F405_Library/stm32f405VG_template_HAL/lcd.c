...
//uint8_t text_bg_color_prev[CHAR_MAX_X][CHAR_MAX_Y];/*for transmit for xbc, msb 4bits: text color, lsb 4bits: bg color*/
...
//   
//   /* Enable TFT_SPI   */
//   _HAL_SPI_ENABLE(TFT_SPI);
//   _SPI_CRCCALCULATION_DISABLE();
...
//   while (__HAL_I2S_GET_FLAG(TFT_SPI, I2S_FLAG_TXE) == RESET);
//   /* Send byte through the TFT_SPI peripheral */
//   SPI_I2S_SendData(TFT_SPI, command);
//   while (__HAL_I2S_GET_FLAG(TFT_SPI, I2S_FLAG_RXNE) == RESET);
