...
static s32 start_counting_dis = 0;
...
GAME_STAGE path_up_sb_update(){
	...
	if (right_angle_ing == 1){
		...
		if (sensorbar_region == HIGH_ORANGE && current_state == 1 && (get_average_dis() - last_encoder_dis) >= SLOPE_SECTION_DIS){
			...
			if (highland_count == 3){
				if (!counting_encoder){
					...
					start_counting_dis = get_average_dis();
				}
			}
		...
		//#ifdef RED_FIELD
		u32 dynamic_river_dis = RIVER_90_TURN_DIS;
		if (counting_encoder && (get_average_dis() - start_counting_dis)>dynamic_river_dis){
			right_angle_ing = 1;
		}
		//#endif
