...
void tft_append_line(const char * pstr, ...){
	...
	if (tft_y_index<0 || tft_y_index>tft_get_max_y_char()){
		tft_y_index = 0;
	}
	...
	while (*fp)	{
		if (*fp == '[') {
			...
			if (x > tft_get_max_x_char() || tft_y_index > (tft_get_max_y_char())) {
				break;
			}
	...
	tft_y_index = tft_y_index > tft_get_max_y_char() ? 0 : tft_y_index+1;
}
