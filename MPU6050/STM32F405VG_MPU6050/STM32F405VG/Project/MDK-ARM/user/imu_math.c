...
static float Rxyz[3] = {0}; //Estimated force vector in x/y/z direction, normalised
static u8 imp = 0; //imu_math_pointer

void calc_init(){
	for (u8 i=0; i<3; i++){
		Rxyz[i] = IMU_Buffer[i]/Abs(IMU_Buffer[i]); //Record the normalised est. force vector
	}
	imp = (imp+1) % 2;
}

void calcIMU(){
	Axy[imp][0] = atan2(Rxyz[0], Rxyz[2]); //int_atan2(Rx, Rz); -> Rxz -> on xz plane with z-axis
	Axy[imp][1] = atan2(Rxyz[1], Rxyz[2]); //int_atan2(Ry, Rz); -> Ryz -> on yz plane with z-axis
	...
	float Rgyro[3] = {0};
	Rgyro[0] = sin(Axy[imp][0]) / sqrt (Sqr(1 + cos(Axy[imp][0])) * tan(Axy[imp][0])^2));
