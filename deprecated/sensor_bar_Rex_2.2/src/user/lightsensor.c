...
void sensor_init(u8 cali_stage){
	...
	for (u8 i=0;i<SAMPELS_TIMES;i++){
		s32 rgb_ratio[16][3];
		for (u8 k=0;k<16;k++){
			s32 rgb_sum = this_readings[i].red_reading[k] + this_readings[i].green_reading[k] + this_readings[i].blue_reading[k];
			
			rgb_ratio[k][0] = 30000*(s32)this_readings[i].red_reading[k]		/(s32)rgb_sum;
			rgb_ratio[k][1] = 30000*(s32)this_readings[i].green_reading[k]	/(s32)rgb_sum;
			rgb_ratio[k][2] = 30000*(s32)this_readings[i].blue_reading[k]		/(s32)rgb_sum;
			
			for (u8 i=0;i<3;i++){
				sum_of_all[i] += rgb_ratio[k][i];
			}
			if (k==7 || k==8) continue;
			for (u8 i=0;i<3;i++){
				sum_of_bg[i] += rgb_ratio[k][i];
			}
		}
		for (u8 i=0;i<3;i++){
			sum_of_mid[i] += rgb_ratio[6][i] +  rgb_ratio[7][i]*2 +  rgb_ratio[8][i]*2 +  rgb_ratio[9][i];
		}
	}
...
void sendData(){
	...
	s32 rgb_ratio[16][3];
	for (u8 i=0;i<16;i++){
		s32 rgb_sum = now.red_reading[i] + now.green_reading[i] + now.blue_reading[i];
		rgb_ratio[i][0] = 30000*(s32)this_readings[i].red_reading[i]		/(s32)rgb_sum;
		rgb_ratio[i][1] = 30000*(s32)this_readings[i].green_reading[i]	/(s32)rgb_sum;
		rgb_ratio[i][2] = 30000*(s32)this_readings[i].blue_reading[i]		/(s32)rgb_sum;
		
		for (u8 k=0;k<3;k++){
			average[k] += rgb_ratio[i][k];
		}
	}
