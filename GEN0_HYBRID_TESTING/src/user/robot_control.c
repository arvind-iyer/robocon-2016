...
** If percentage mode is enabled, PWM mode will be used
**/

u16 brushless_percentage[2] = {0};
bool brushless_using_pwm = false;
u16 brushless_pwm_count[2] = {0};

void brushless_update(){
	if (brushless_using_pwm){
		for (u8 i=0;i<2;i++){
			if (brushless_percentage[i]<32 && brushless_percentage[i]>20){
				brushless_pwm_count[i]++;
				if (brushless_pwm_count[i]>20){
					brushless_pwm_count[i]=0;
					brushless_control((BRUSHLESS_ID)i, 0, true);
				}else if (brushless_pwm_count[i]>(20*(35-brushless_percentage[i])/35+9)){
					brushless_control((BRUSHLESS_ID)i, 32, true);
				}
			}
		}
	}
...
void brushless_control(BRUSHLESS_ID brushless_id, u16 value, bool is_percentage_mode){
	...
	brushless_using_pwm = is_percentage_mode;
	if (is_percentage_mode){
		value = value>100?100:value;
		brushless_percentage[brushless_id] = value;
