...
void manual_interval_update(){
	...
	*/
	if (brushless_lock == UNLOCKED){
		...
		u16 brushless_power = 5;
		brushless_power += xbc_get_joy(brushless_joy_sticks[0])*2/5;
		brushless_power += xbc_get_joy(brushless_joy_sticks[1])*2/50;
		tft_append_line("%d", brushless_power);
		brushless_control_all(brushless_power, true);
