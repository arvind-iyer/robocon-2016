...
volatile u32 ticks = 0; //1 ticks 50us
...
u16 get_ms_ticks(void) {
	return ticks*50/1000;
}

u16 get_seconds(void){
return ticks*50/1000/1000;
}
...
//	TICKS_TIM->CR1 = 1;
	...
	TIM_TimeBaseStructure.TIM_Period = 50;	                 				       // Timer period, x ticks in one clock frequency
...
u16 song_index = 0;
u16 closer = 0;
const MUSIC_NOTE_DUR* playing_song;
...
TICKS_IRQHandler
{
  if (TIM_GetITStatus(TICKS_TIM, TIM_IT_Update) != RESET) {
    TIM_ClearFlag(TICKS_TIM, TIM_FLAG_Update);
			ticks++;
			if (playing == 1 && ticks%20 == 0){
				dur -= 1;
				buzzer_check();
				if (dur<=0){
					song_index ++;
					if (playing_song[song_index].note==NOTE_REST){
						buzzer_off();
					}else if(playing_song[song_index].note==NOTE_END){
						playing = 0;
					}else{
						buzzer_control_note(1,100,playing_song[song_index].note, playing_song[song_index].octave);
					}
					dur = playing_song[song_index].duration;
					closer = 100;
				}
			}
		if (closer--<0){
...
void playSong(const MUSIC_NOTE_DUR* song){
	playing = 1;
	song_index = 0;
	buzzer_control(1,song[song_index].note);
	dur = song[0].duration;
	song_index ++ ;
	playing_song = song;
}
