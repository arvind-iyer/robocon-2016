...
static s32 ap_start_X = 0, ap_start_Y = 0, ap_start_angle;
...
u8 limit_sa_approach(s16 motor_vel[3]){
	...
	if (abs(get_ap_new_X()) > LM_SA_APP_START_ROTATE_X){
		...
		//angle pid
		s16 this_angle_error =  abs(target_angle - get_angle())%3600;
	...
	s16 target_new_y = (LM_SA_APP_CURVE_RADIUS - ((s32)u32_sqrt(abs(LM_SA_APP_CURVE_RADIUS*LM_SA_APP_CURVE_RADIUS 
											- (get_ap_new_X())*(get_ap_new_X())))))
											* LM_SA_APP_CURVE_GAIN /100;
	#ifdef BLUE_FIELD
		target_new_y = -target_new_y;
	#endif
	s16 y_error = get_ap_new_Y() - target_new_y;
