...
float path_yaw[] = {0.0, 15.0, 15.0};
float path_pitch[] = {0.0, -6.5, 0.0};
uint8_t path_pointer = 0;

float rolling_pitch[ROLLING_PITCH_SIZE] = {0};
uint8_t rolling_pitch_index = 0;
uint8_t progress_count = 0;

uint8_t current_stage = 0;
float targeting_yaw;
float awaiting_pitch;
...
void path_update(){
	...
	rolling_pitch = rolling_pitch % ROLLING_PITCH_SIZE;
