...
void init_all_zero(){
	for(u8 i = 0 ; i < 16 ; i++){
		...
		full_white_reading.off_reading[i] = 255;
		full_white_reading.red_reading[i] = 255;
		full_white_reading.green_reading[i] = 255;
		full_white_reading.blue_reading[i] = 255;
	}
...
void sensor_cali(u8 cali_stage){
	...
	for (u8 i=0;i<3;i++){
		compensated_region_color[cali_stage][i] = reading_in_area[cali_stage][1][i] + (reading_in_area[cali_stage][0][i] - reading_in_area[cali_stage][1][i])*3/4;
