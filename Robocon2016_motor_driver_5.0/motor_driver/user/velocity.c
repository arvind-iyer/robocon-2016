...
{
	target_vel = vel;				/* Origin: target_vel = vel;
															changed here for turning to correct direction*/
...
//    } else {
      // Use pwm control and reset timeout.
		curr_pwm += PID; /* Origin: curr_pwm += PID;
												Changed here for turning in right direction*/
...
{
	return current_vel - get_encoder_vel(); /*Origin:  current_vel - get_encoder_vel();*/
}
