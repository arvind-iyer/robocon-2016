...
{
	if (dir == ANTI_CKW) {
		GPIO_ResetBits(MOTOR_DIR_GPIOx, MOTOR_DIR_Pin); /*Origin GPIO_ResetBits(MOTOR_DIR_GPIOx,MOTOR_DIR_Pin);
																										Changed here for turning in right drection*/
	} else if (dir == CKW) {
...
{
	//pwm = -pwm; //inverse the sign here for turning in correct direction
	if (pwm > MAX_PWM) {
		pwm = MAX_PWM;
